...
export class GalleryController {
  ...
  async addImage(req: Request, res: Response) {
    ...
    if (!name || !image || !category) {
      return res
        ...
        .json({ message: "Name, image, and category are required." });
    }
  ...
  async addgalleryCategory(req: Request, res: Response) {
    ...
    try {
      const existingCategory = await this.galleryUseCase.findCategoryByName(
        categoryName
      );
      ...
      const savedGalleryCategory = await this.galleryUseCase.addGalleryCategory(
  ...
  async onSearch(req: Request, res: Response) {
    ...
    try {
      ...
      res
        .status(HttpStatusCode.INTERNAL_SERVER_ERROR)
        .json({ message: "Error searching gallery" });
    }
  }
  ...
  async editGalleryImage(req: Request, res: Response) {
    try {
      ...
      const updatedGallery = await this.galleryUseCase.updateGalleryData(
        galleryId,
        galleryData
      );
      if (!updatedGallery) {
      ...
      return res
        .status(HttpStatusCode.INTERNAL_SERVER_ERROR)
        .json({ message: "Failed to update gallery item" });
    }
  }
  ...
  async deleteGalleryData(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      res
        ...
        .send({ message: "Gallery deleted successfully" });
    } catch (error) {
      res
        .status(HttpStatusCode.INTERNAL_SERVER_ERROR)
        .send({ message: "Error deleting gallery", error });
    }
  }
  ...
  async getGalleryCategoryData(req: Request, res: Response): Promise<void> {
    try {
      ...
      res
        .status(HttpStatusCode.INTERNAL_SERVER_ERROR)
        .json({ message: "Gallery category data fetching error", error });
    }
  }
